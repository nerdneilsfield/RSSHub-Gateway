...
func TestShortRedirectInternal(t *testing.T) {
	cfg := `server:
  listen: ":0"
  timeout_ms: 200
...
`
	...
	if location != "/rsshub/latepost/4?key=K" {
		t.Fatalf("unexpected location: %s", location)
	}
}

func TestShortRedirectExternal(t *testing.T) {
	cfg := `server:
  listen: ":0"
  timeout_ms: 200
...
func TestShortNotFound(t *testing.T) {
	cfg := `server:
  listen: ":0"
  timeout_ms: 200
